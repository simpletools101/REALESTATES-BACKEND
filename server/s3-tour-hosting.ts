...
console.log('AWS_ACCESS_KEY_ID first 4 chars:', process.env.AWS_ACCESS_KEY_ID?.substring(0, 4))
console.log('AWS_SECRET_ACCESS_KEY exists:', !!process.env.AWS_SECRET_ACCESS_KEY)
console.log('S3_TOURS_BUCKET:', process.env.S3_TOURS_BUCKET)
console.log('==============================')

// Validate credentials exist
if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
    throw new Error('AWS credentials are required. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY in .env file')
}

if (!process.env.AWS_REGION) {
    throw new Error('AWS_REGION is required. Please set it in .env file (e.g., eu-central-1)')
}
    ...
    try {
        ...
        try {
            ...
            if (statusCode === 404) {
                ...
                console.error('❌ Access Denied to bucket. Details:', {
                    bucket: BUCKET_NAME,
                    region: process.env.AWS_REGION,
                ...
                throw new Error(
                    ...
                    `This usually means:\n` +
                    `1. Bucket exists in different AWS account\n` +
                    `2. IAM user lacks bucket permissions\n` +
                    `3. Bucket policy denies access\n` +
                    `Solution: Create a new bucket with unique name or update bucket policy`
                )
            } else if (statusCode === 301) {
                throw new Error(
                    `Bucket "${BUCKET_NAME}" exists in a different region than ${process.env.AWS_REGION}. ` +
                    `Run: aws s3api get-bucket-location --bucket ${BUCKET_NAME} to find correct region`
                )
            } else {
                throw error
            }
        ...
        try {
            ...
            console.warn('   Tours may not be publicly accessible without this policy')
        }

        console.log('✅ S3 bucket configured successfully for tour hosting')
    } catch (error: any) {
        console.error('❌ Error setting up S3 bucket:', {
            message: error.message,
            statusCode: error.$metadata?.httpStatusCode,
            requestId: error.$metadata?.requestId,
        })
        throw new Error(`Failed to setup S3 bucket: ${error.message}`)
    }
}
...
function collectAllFiles(
    dir: string,
    basePath: string = ''
): Array<{ localPath: string; s3Key: string; size: number }> {
    ...
    try {
        ...
        console.error(`Error collecting files from ${dir}:`, error.message)
    }
    return files
}
    ...
    try {
        ...
        const findIndexFile = (dir: string): string | null => {
            ...
            for (const entry of entries) {
                ...
                if (
                    entry.isFile() &&
                    (entry.name.toLowerCase() === 'index.html' || entry.name.toLowerCase() === 'index.htm')
                ) {
        ...
        for (const file of files) {
            try {
                ...
                console.error(`✗ Failed to upload ${path.basename(file.localPath)}:`, error.message)
                throw error
            }
        }
        ...
        const tourUrl = `https://${BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${indexS3Key}`
...
export async function initializeS3() {
    ...
    if (!process.env.AWS_REGION) {
        console.error('❌ AWS_REGION not set in environment variables')
        console.error('   Please set AWS_REGION in .env file (e.g., AWS_REGION=eu-central-1)')
        return
    }
    ...
    try {
        ...
        console.error('❌ Failed to initialize S3 tour hosting:', error.message)
    }
}